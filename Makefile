...
fuzz-release:
	go test $(FUZZ_PACKAGE) -run '^$$' -fuzz=$(FUZZ_TARGET) -fuzztime=100000x

fuzz-nightly:
	go test $(FUZZ_PACKAGE) -run '^$$' -fuzz=$(FUZZ_TARGET) -fuzztime=1000000x
...
prepare-release-version:
	./scripts/prepare-release-version.sh --version "$(VERSION)"

create-signed-release-tag:
	@test -n "$(RELEASE_NOTES)" || { echo "RELEASE_NOTES=/path/to/release-notes.md is required" >&2; exit 2; }
	./scripts/create-signed-release-tag.sh --version "$(VERSION)" --notes "$(RELEASE_NOTES)" $${PWNED_CHECK_RELEASE_TAG_FLAGS:-}

private-signing-material-check:
	./scripts/check-no-private-signing-material.sh .
...
package-repository-pages-publish-test:
	./scripts/publish-package-repositories-to-pages-test.sh

native-pam-repo-endpoint-retry-test:
	./scripts/native-pam-repo-endpoint-check-retry-test.sh
...
native-pam-repositories-docker:
