...
|---|---|---|---|---|
| Debian/Ubuntu | `/lib/$DEB_HOST_MULTIARCH/security/pam_pwned_check.so` | `make package-native-pam-debian` | `make native-pam-ubuntu-deb-package-smoke` | Package `pwned-check-native-pam` installs a `pam-auth-update` profile at `/usr/share/pam-configs/pwned-check`; operators use `pwned-check-pam-enable-dry-run`, `pwned-check-pam-enable-enforce`, and `pwned-check-pam-disable`. Release automation builds `amd64` and `arm64` packages in Rust Debian containers. Run the smoke on `codex-vm-debian` with `PATH="/usr/sbin:$PATH"` so Debian SSH sessions can find `pam-auth-update`. |
