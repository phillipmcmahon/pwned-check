...
check_arch() {
    ...
    desc_path="$(tar -tf "$arch_dir/pwned-check.db" | awk -F/ -v pkg="$PACKAGE_NAME" '$1 ~ "^" pkg "-" && $2 == "desc" { print $0; exit }')"
    [ -n "$desc_path" ] || fail "Arch repository database does not include a desc entry for $PACKAGE_NAME"
    tar -xOf "$arch_dir/pwned-check.db" "$desc_path" > "$arch_dir/desc"
...
check_alpine() {
    ...
    for alpine_arch in x86_64 aarch64; do
        arch_dir="$alpine_dir/$alpine_arch"
        mkdir -p "$arch_dir"
        fetch "$BASE_URL/alpine/$alpine_arch/APKINDEX.tar.gz" "$arch_dir/APKINDEX.tar.gz"
        tar -tzf "$arch_dir/APKINDEX.tar.gz" > "$arch_dir/list"
        grep -E '^\.SIGN\.RSA\.' "$arch_dir/list" >/dev/null || fail "Alpine $alpine_arch APKINDEX is missing embedded RSA signature entry"
        tar -xzf "$arch_dir/APKINDEX.tar.gz" -C "$arch_dir"
        alpine_sig="$(find "$arch_dir" -name '.SIGN.RSA.*' -print -quit)"
        [ -n "$alpine_sig" ] || fail "Alpine $alpine_arch APKINDEX signature entry could not be extracted"
        assert_contains "$arch_dir/APKINDEX" "P:$PACKAGE_NAME"
