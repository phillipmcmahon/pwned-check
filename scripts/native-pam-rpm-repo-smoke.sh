...
if [ "${PWNED_CHECK_RPM_REPO_SMOKE_NO_CAPTURE:-}" != "1" ]; then
    timestamp="$(date -u '+%Y%m%dT%H%M%SZ')"
    run_dir="$OUTPUT_ROOT/$timestamp-native-pam-rpm-repo-smoke"
    mkdir -p "$run_dir"
    log_file="$run_dir/$timestamp-native-pam-rpm-repo-smoke.txt"
    rc_file="$run_dir/exit-code"
    (
        set +e
        PWNED_CHECK_RPM_REPO_SMOKE_NO_CAPTURE=1 "$0" "$@"
        rc=$?
        printf '%s\n' "$rc" > "$rc_file"
        exit "$rc"
    ) 2>&1 | tee "$log_file"
    ...
    printf 'Native PAM RPM repository smoke output: %s\n' "$log_file"
    exit "$rc"
fi
