...
if [ -n "$PWNED_CHECK_BIN" ]; then
    artifact_name="$(cd "$ROOT" && ./scripts/lib/package-native-pam-debian-rootfs.sh --version "$VERSION" --output-dir "$ARTIFACT_OUT" --build-time "$BUILD_TIME" --pwned-check-bin "$PWNED_CHECK_BIN")"
else
    artifact_name="$(cd "$ROOT" && ./scripts/lib/package-native-pam-debian-rootfs.sh --version "$VERSION" --output-dir "$ARTIFACT_OUT" --build-time "$BUILD_TIME")"
fi
